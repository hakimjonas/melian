...
Status of the subsections below: static files, health/readiness, CSRF protection, session management, rate limiting, and the graceful-shutdown wrapper are implemented as `melian-server` middleware. CORS, request logging, request IDs, authentication, error handling, compression, and body limits are provided by `eru-http`'s `Middleware`; the fluent builder API sketched in the section bodies is descoped (see Section 13) -- compose these middlewares through `MelianServer.serveWith` instead. ACME is provisioned by eru-http's `eru-http-acme` module (1.0.0-alpha.2; TLS is that library's domain); Melian wires the result via `MelianServer.withAcme`.

### 14.1 Let's Encrypt / ACME Provisioning

**Implemented** in `melian-server` on top of eru-http's `eru-http-acme` module (1.0.0-alpha.2), which owns the protocol; Melian wires the result into `HttpServerConfig.withTls`.

```scala
MelianServer.withAcme(
  MelianServer.AcmeSettings(
    domain = "api.example.com",
    contactEmail = "admin@example.com",
    staging = false,                // true for Let's Encrypt staging
    storePath = Path.of("./certs"), // persistent certificate storage
  ),
  HttpServerConfig.localhost,
  router,
) { (server, acme) =>
  // acme.challengeAddress: the HTTP-01 listener; acme.provisioner.tlsConfig: the produced config
  server.start.map(println)
}
```

Under the hood: `eru-http-acme` runs the RFC 8555 client (ES256 JWS, nonce management), persists the account key and issued material, and builds a PKCS12 keystore; the renewal loop re-issues before expiry and swaps the produced `TlsConfig`. The HTTP-01 challenge responder answers `/.well-known/acme-challenge/<token>` on the listener `withAcme` starts on `http01Port` (ACME validates on port 80). A still-valid stored certificate is reused without ACME traffic, so restarts are cheap. Melian doesn't manage TLS itself.
...
Over-limit requests answer 429 Too Many Requests via eru-http's `Response.tooManyRequests`, with a
`Retry-After` header carrying the seconds left in the window. The key is whatever the `keyExtractor`
derives; the default keys on the resolved client address that eru-http 1.0.0-alpha.2 exposes as
`Request.clientAddress` (TCP peer, PROXY-protocol-derived, or trusted-XFF-derived), falling back to a
global key for requests without one. The in-memory store bounds its key
