...
sbt check         # doc coverage + scalafix --check + scalafmt (the CI gate)
sbt testAll       # every module, full run
```
...
baseline, and `check` fails when a file's undocumented count grows (a new member without a
doc, or a doc removed). Documenting members never fails.
