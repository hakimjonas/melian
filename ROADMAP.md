...
| `eru-http` | 1.0.0-alpha.2 | HTTP client and server, RFC 6455 WebSockets, ACME (`eru-http-acme`) |
| `sarati` | 1.0.0-alpha.2 | Binary codec, format ASTs, XPath evaluator |
| `rumil` | 1.0.0-alpha.5 | Parser combinators, lossless green/red trees |
...
- `RateLimit`: fixed-window limiting per key (default key: the resolved client address), pluggable `RateLimitStore`, 429 with `Retry-After`.
...
No blockers remain. Every upstream item from the ecosystem review has landed and is published to
Maven Central, and the adoption of the full surface is on the `adopt-upstream-alpha2` branch:
...
1. Land the adoption (this branch).
2. Re-cut `v1.0.0-alpha` on the final commit (the earlier tag and its failed release run were
   removed — nothing was ever published, so the version name is clean).
...
PGP secrets are already configured on the repository. Changes land via PR; nothing publishes
without a tag push, and tags come only from the sequence above.
